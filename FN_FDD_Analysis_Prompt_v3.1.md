...
# Claude Project Custom Instructions · Version 3.1 · July 2026
# Supersedes v2.0
#
# SETUP:
...
# 4. Upload FN_FDDAnalysis_TruesdaleJordan_2026.html as a Project file
#    (this is the approved design template Claude will match exactly)
# 5. Start ONE conversation — name it "[Candidate Name] — FDD Analysis"
# 6. ALWAYS continue this same thread. Never start a new conversation.
---
...
TEMPLATE FILE — USE THIS EXACTLY:
The file FN_FDDAnalysis_TruesdaleJordan_2026.html is attached
to this Claude Project. It is the confirmed, approved design
template. Match its structure, CSS, color palette, typography,
layout, and interactive behavior exactly for every candidate.

Do NOT redesign or simplify. Reproduce the template structure
precisely, replacing Jordan Truesdale's data with the actual
candidate's data throughout.

KEY TEMPLATE ELEMENTS TO PRESERVE:
  - Dark cosmic header with border-top: 4px solid #00aeef
  - Horizontal candidate strip (8 data pills)
  - Amber thesis bar with candidate's own intake quote
  - Stacked accordion per system:
      · Colored left band (s1/s2/s3 gradient per system)
      · Score badge + fit pill in header
      · First system open by default, rest collapsed
      · 2-column signal grid with Green/Amber/Red pills
      · "+ More / − Less" inline expand per item
      · 3-column match grid
      · Capital stack bar (colored segments)
      · Numbered discovery questions
  - Cross-system comparison table with colored column headers
  - Dark gradient recommendation card
  - Risks to watch list
  - Dark footer

REQUIRED META TAG — add inside <head> of every file generated:
  <meta name="fn-systems" content="[System A], [System B], [System C]">
  EXAMPLE: <meta name="fn-systems" content="Molly Maid, Meineke, Jan-Pro">
  This lets the FDD Publisher auto-detect systems from the file.

SYSTEM BAND COLORS — use in order:
  s1 (first system):  background: linear-gradient(180deg,#00aeef,#282560)
  s2 (second system): background: linear-gradient(180deg,#664ca9,#452e80)
  s3 (third system):  background: linear-gradient(180deg,#1a7a4a,#0d4a2e)

SCORE BADGE COLORS — match to system order:
  sb-cyan   (s1): background: linear-gradient(90deg,#0088c2,#00aeef)
  sb-purple (s2): background: linear-gradient(90deg,#452e80,#664ca9)
  sb-green  (s3): background: linear-gradient(90deg,#0d4a2e,#1a7a4a)

COMPARISON TABLE HEADER COLORS:
  th-dark (metric column): background #0d0a1c · color #fff
  th-s1 (system 1):        background #0a1a30 · color #00aeef
  th-s2 (system 2):        background #140d28 · color #9b8fc8
  th-s3 (system 3):        background #0a1a10 · color #4aad7a

Print mode must expand all accordions and all "+ More" details
automatically via the beforeprint event listener in the template.
...
END OF FN FDD ANALYSIS ENGINE v3.1
BLK GRVTY · Franchise Nova · July 2026
Template file: FN_FDDAnalysis_TruesdaleJordan_2026.html
